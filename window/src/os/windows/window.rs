...
impl WindowInner {
  pub(super) fn accessibility_action(
    ...
    let bridge = match self.accessibility.as_ref() {
      Some(bridge) if std::sync::Weak::ptr_eq(
        &lifetime, &std::sync::Arc::downgrade(&bridge.lifetime),
      ) => bridge,
      _ => return,
    };
    ...
    if request.action == accesskit::Action::Focus {
      ...
      promise::spawn::spawn(async move {
        if lifetime.upgrade().is_some() {
          unsafe { SetFocus(hwnd.0); }
        }
      }).detach();
    } else if unsafe { GetFocus() == self.hwnd.0 } {
      if let Some(text) = input_text(request) {
        self.events.dispatch(WindowEvent::AccessibilityInput { pane_id, text });
      }
    }
  }
...
impl WindowOps for Window {
  ...
  fn set_accessibility_input_target(&self, pane_id: usize, cursor: Rect) {
    Connection::with_window_inner(self.0, move |inner| {
...
unsafe fn wm_getobject(hwnd: HWND, wparam: WPARAM, lparam: LPARAM) -> Option<LRESULT> {
  ...
  let result = {
    ...
    bridge.adapter.handle_wm_getobject(
      accesskit_windows::WPARAM(wparam), accesskit_windows::LPARAM(lparam),
      &mut bridge.state,
    )
  };
